...
const StatusBadge = ({ status }) => {
    const styles = {
        ...
        'cancelled': 'bg-[var(--bg-overlay)] text-[var(--text-muted)] border-[var(--border-strong)] line-through'
    };
    const labels = { won: 'Ganada', lost: 'Perdida', pending: 'Pendiente', void: 'Reembolsada', 'half-won': 'Mitad Ganada', 'half-lost': 'Mitad Perdida', cancelled: 'Cancelada' };
...
const buildOfficialPickEntryLink = (pickId, { register = false } = {}) => {
    const params = new URLSearchParams();
    params.set('addPick', pickId);
...
const OfficialPickTrustNote = () => <div className="rounded-2xl border border-[var(--border)] bg-[var(--accent-5)] p-4 text-sm text-[var(--text-muted)]"><p className="font-bold text-[var(--text-main)]">Cómo leer este comprobante</p><ul className="mt-2 space-y-1 list-disc pl-5"><li>La hora y la cuota son las registradas por el servidor al publicar el pick.</li><li>El hash identifica la evidencia registrada: no es una garantía de resultado ni una recomendación personalizada.</li><li>Si se reporta una incidencia, el pick original se conserva y la revisión queda visible en su historial.</li></ul></div>;

const OfficialPicksPanel = ({ currentBank, currentUser, isPremium, copiedPickIds, copyingPickId, onCopy, focusPickId, onCreateFirstBank }) => {
    ...
    const focusedPickRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                const [listResponse, focusedResponse] = await Promise.all([
                    fetch('/api/official-picks?limit=20', { headers: { Authorization: `Bearer ${token}` } }),
                    focusPickId ? fetch(`/api/official-picks?pick=${encodeURIComponent(focusPickId)}`, { headers: { Authorization: `Bearer ${token}` } }) : Promise.resolve(null)
                ]);
                ...
                if (!listResponse.ok) throw new Error(data.error || 'No se pudieron cargar los picks oficiales.');
                let loadedPicks = Array.isArray(data.picks) ? data.picks : [];
                if (focusedResponse?.ok && focusedResponse.headers.get('content-type')?.includes('application/json')) {
                    const focusedData = await focusedResponse.json();
                    if (focusedData.pick && !loadedPicks.some((pick) => pick.id === focusedData.pick.id)) {
                        loadedPicks = [focusedData.pick, ...loadedPicks];
                    }
                }
                if (active) setPicks(loadedPicks);
            } catch (requestError) {
        ...
        load();
        return () => { active = false; };
    }, [currentUser, focusPickId, isPremium]);

    useEffect(() => {
        if (!loading && focusPickId && focusedPickRef.current) {
            focusedPickRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
    }, [loading, focusPickId, picks.length]);
    ...
    return <div className="space-y-4">
        {!isPremium && <div className="rounded-2xl border border-[var(--accent-30)] bg-[var(--accent-5)] p-4 text-sm text-[var(--text-main)]"><p className="font-extrabold flex items-center gap-2"><Crown size={17} className="text-[var(--accent)]"/>Acceso Money Tips Premium</p><p className="mt-1 text-[var(--text-muted)]">Los picks activos conservan su sello público, pero mercado, selección y cuota se muestran solo a miembros Premium hasta el inicio del partido.</p><a href="https://pronosticosmoneytips.com/#premium" className="inline-flex mt-3 text-[var(--accent)] font-bold">Conocer Premium <ArrowUpRight size={15} className="ml-1"/></a></div>}
        {focusPickId && <div className="bg-[var(--accent-10)] border border-[var(--accent-30)] rounded-2xl p-4 text-sm text-[var(--text-main)]"><p className="font-bold text-[var(--accent)]">Tu pick seleccionado está resaltado abajo.</p><p className="text-[var(--text-muted)] mt-1">Revisa la cuota y añádelo solo a una banca individual que hayas configurado.</p></div>}
        {picks.map((pick) => {
            const copied = copiedPickIds.has(pick.id);
            const hasFullPickDetails = Boolean(pick.bet?.market && pick.bet?.selection);
            const hasHistoricalOdds = Number.isFinite(Number(pick.bet?.oddsAtPublication));
            const cannotCopy = !currentBank || currentBank.isBalance || !hasFullPickDetails;
            return <article key={pick.id} ref={pick.id === focusPickId ? focusedPickRef : null} className={`bg-[var(--bg-card)] border rounded-3xl p-5 md:p-6 shadow-sm ${pick.id === focusPickId ? 'border-[var(--accent)] ring-2 ring-[var(--accent-20)]' : 'border-[var(--border)]'}`}>
                <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                    <div className="min-w-0">
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3 max-w-lg">
                            <div className="bg-[var(--bg-input)] rounded-xl p-3">
                                <p className="text-[10px] uppercase tracking-wider font-bold text-[var(--text-muted)]">Pronóstico</p>
                                {hasFullPickDetails
                                    ? <><p className="font-bold text-[var(--text-main)] text-sm mt-1">{pick.bet.market}</p><p className="text-[var(--accent)] text-sm mt-1">{pick.bet.selection}</p></>
                                    : <p className="font-bold text-[var(--text-muted)] text-sm mt-1">Detalle reservado para Premium</p>}
                            </div>
                            <div className="bg-[var(--bg-input)] rounded-xl p-3">
                                <p className="text-[10px] uppercase tracking-wider font-bold text-[var(--text-muted)]">Cuota publicada</p>
                                {hasHistoricalOdds
                    ...
                    <div className="flex flex-col gap-2 shrink-0 md:w-52">
                        <a href={buildOfficialPickLink(pick.id)} target="_blank" rel="noreferrer" className="text-center px-4 py-2.5 rounded-xl border border-[var(--border)] text-sm font-bold text-[var(--text-main)] hover:bg-[var(--bg-hover)]">Ver comprobante</a>
                        <button onClick={() => onCopy(pick)} disabled={copied || cannotCopy || copyingPickId === pick.id} className="px-4 py-3 rounded-xl text-sm font-extrabold bg-[var(--accent)] text-[var(--accent-fg)] disabled:opacity-50 disabled:cursor-not-allowed">{!hasFullPickDetails ? 'Acceso Premium necesario' : copied ? 'Añadido a tu banca' : copyingPickId === pick.id ? 'Añadiendo...' : 'Añadir a mi banca'}</button>
                        {!currentBank && onCreateFirstBank && hasFullPickDetails && <button onClick={onCreateFirstBank} className="px-4 py-2.5 rounded-xl border border-[var(--accent-30)] text-sm font-bold text-[var(--accent)] hover:bg-[var(--accent-10)]">Crear mi primera banca</button>}
                        <button onClick={() => { setReportPick(pick); setReportCategory(''); setReportError(''); }} className="px-4 py-2.5 rounded-xl border border-yellow-500/30 text-sm font-bold text-[var(--text-main)] hover:bg-yellow-500/10">Reportar incidencia</button>
                        {!hasFullPickDetails && <p className="text-[10px] text-[var(--text-muted)] text-center">Mercado y selección reservados para Premium.</p>}
                        {hasFullPickDetails && cannotCopy && <p className="text-[10px] text-[var(--text-muted)] text-center">{currentBank?.isBalance ? 'Selecciona una banca individual para copiarlo.' : 'Configura una banca individual para copiarlo.'}</p>}
                    </div>
                </div>
            </article>;
        })}
...
const OfficialPickPublicPage = ({ pickId, theme }) => {
    ...
    if (pick?.isLocked) {
        return <><style>{getGlobalStyles(theme)}</style><LiquidBackground theme={theme}/><main className="min-h-screen p-4 md:p-8 flex items-center justify-center"><section className="w-full max-w-2xl bg-[var(--bg-card)] border border-[var(--border)] rounded-[2rem] shadow-[var(--shadow-glow-md)] overflow-hidden"><header className="p-6 md:p-8 border-b border-[var(--border)]"><div className="flex items-center gap-2 text-[var(--accent)] font-bold text-xs uppercase tracking-widest"><ShieldCheck size={16}/> Comprobante oficial Money Tips</div><h1 className="mt-3 text-2xl md:text-3xl font-extrabold text-[var(--text-main)]">{pick.event.homeTeam} vs {pick.event.awayTeam}</h1><p className="text-sm text-[var(--text-muted)] mt-2">Registro sellado por servidor. Este documento no se puede editar desde la aplicación.</p></header><div className="p-6 md:p-8 space-y-6"><div className="bg-[var(--bg-input)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Pronóstico</p><p className="font-bold text-[var(--text-main)] mt-2">Detalle reservado para Premium</p><p className="text-sm text-[var(--text-muted)] mt-1">El mercado, la selección y la cuota estarán disponibles para miembros Premium hasta el inicio.</p></div><div className="grid sm:grid-cols-2 gap-4"><div className="bg-[var(--bg-input)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Inicio</p><p className="font-bold text-[var(--text-main)] mt-2">{formatOfficialDateTime(pick.event.kickoffAt)}</p></div><div className="bg-[var(--bg-input)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Publicado</p><p className="font-bold text-[var(--accent)] mt-2">{formatOfficialDateTime(pick.publishedAt)}</p></div></div><div className="border border-[var(--border)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Evidencia</p><p className="text-xs font-mono break-all text-[var(--text-muted)] mt-2">SHA-256: {pick.source.evidenceHash}</p><p className="text-xs text-[var(--text-muted)] mt-2">Este hash queda publicado antes del inicio y protege la integridad del registro.</p></div></div><footer className="p-6 border-t border-[var(--border)] space-y-3"><a href={buildOfficialPickEntryLink(pick.id, { register: true })} className="block text-center bg-[var(--accent)] text-[var(--accent-fg)] rounded-xl py-3 font-bold">Crear cuenta y conocer Premium</a><a href={window.location.pathname} className="block text-center text-sm text-[var(--text-muted)] font-bold">Ver MoneyTracKING</a></footer></section></main></>;
    }

/* Legacy variant retained in the merge history:
    return <><style>{getGlobalStyles(theme)}</style><LiquidBackground theme={theme}/><main className="min-h-screen p-4 md:p-8 flex items-center justify-center"><section className="w-full max-w-2xl bg-[var(--bg-card)] border border-[var(--border)] rounded-[2rem] shadow-[var(--shadow-glow-md)] overflow-hidden"><header className="p-6 md:p-8 border-b border-[var(--border)]"><div className="flex items-center gap-2 text-[var(--accent)] font-bold text-xs uppercase tracking-widest"><ShieldCheck size={16}/> Comprobante oficial Money Tips</div><h1 className="mt-3 text-2xl md:text-3xl font-extrabold text-[var(--text-main)]">{pick ? `${pick.event.homeTeam} vs ${pick.event.awayTeam}` : 'Verificando pick'}</h1><p className="text-sm text-[var(--text-muted)] mt-2">Registro sellado por servidor. Este documento no se puede editar desde la aplicación.</p></header><div className="p-6 md:p-8">{error ? <div className="text-[var(--red)] bg-[var(--red-10)] border border-[var(--red-30)] rounded-2xl p-5 text-sm">{error}</div> : !pick ? <div className="text-center text-[var(--text-muted)] py-10">Cargando comprobante...</div> : <div className="space-y-6"><div className="grid sm:grid-cols-2 gap-4"><div className="bg-[var(--bg-input)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Pronóstico</p>{pick.isLocked ? <><p className="font-bold text-[var(--text-main)] mt-2">Detalle reservado para Premium</p><p className="text-sm text-[var(--text-muted)] mt-1">Se revelará públicamente al inicio del partido.</p></> : <><p className="font-bold text-[var(--text-main)] mt-2">{pick.bet.market}</p><p className="text-[var(--accent)] font-bold mt-1">{pick.bet.selection} · @{Number(pick.bet.oddsAtPublication).toFixed(2)}</p></>}</div><div className="bg-[var(--bg-input)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Horario</p><p className="font-bold text-[var(--text-main)] mt-2">Inicio: {formatOfficialDateTime(pick.event.kickoffAt)}</p><p className="text-[var(--accent)] text-sm mt-1">Publicado: {formatOfficialDateTime(pick.publishedAt)}</p></div></div><div className="border border-[var(--border)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Evidencia</p>{!pick.isLocked && <p className="text-sm text-[var(--text-main)] mt-2">Sistema: {pick.system.id} · versión {pick.system.version}</p>}<p className="text-xs font-mono break-all text-[var(--text-muted)] mt-2">SHA-256: {pick.source.evidenceHash}</p><p className="text-xs text-[var(--text-muted)] mt-2">Este hash queda publicado antes del inicio y permite comprobar el registro cuando el detalle se revele.</p></div><OfficialReviewStatus review={pick.review} /><div><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)] mb-3">Historial del registro</p><div className="space-y-2">{events.map((event) => <div key={event.id} className="flex items-center justify-between gap-3 text-sm bg-[var(--bg-input)] rounded-xl px-4 py-3"><div><p className="font-bold text-[var(--text-main)]">{eventLabel(event)}</p>{event.message && <p className="text-xs text-[var(--text-muted)] mt-1">{event.message}</p>}{event.permalink && <a href={event.permalink} target="_blank" rel="noreferrer" className="inline-block text-xs text-[var(--accent)] font-bold mt-1">Ver mensaje de Telegram</a>}</div><span className="text-[var(--text-muted)] text-xs shrink-0">{formatOfficialDateTime(event.createdAt)}</span></div>)}</div></div></div>}</div><footer className="p-6 border-t border-[var(--border)]"><a href={window.location.pathname} className="block text-center bg-[var(--accent)] text-[var(--accent-fg)] rounded-xl py-3 font-bold">Abrir MoneyTracKING</a></footer></section></main></>;
*/
    return <><style>{getGlobalStyles(theme)}</style><LiquidBackground theme={theme}/><main className="min-h-screen p-4 md:p-8 flex items-center justify-center"><section className="w-full max-w-2xl bg-[var(--bg-card)] border border-[var(--border)] rounded-[2rem] shadow-[var(--shadow-glow-md)] overflow-hidden"><header className="p-6 md:p-8 border-b border-[var(--border)]"><div className="flex items-center gap-2 text-[var(--accent)] font-bold text-xs uppercase tracking-widest"><ShieldCheck size={16}/> Comprobante oficial Money Tips</div><h1 className="mt-3 text-2xl md:text-3xl font-extrabold text-[var(--text-main)]">{pick ? `${pick.event.homeTeam} vs ${pick.event.awayTeam}` : 'Verificando pick'}</h1><p className="text-sm text-[var(--text-muted)] mt-2">Registro sellado por servidor. Este documento no se puede editar desde la aplicación.</p></header><div className="p-6 md:p-8">{error ? <div className="text-[var(--red)] bg-[var(--red-10)] border border-[var(--red-30)] rounded-2xl p-5 text-sm">{error}</div> : !pick ? <div className="text-center text-[var(--text-muted)] py-10">Cargando comprobante...</div> : <div className="space-y-6"><div className="grid sm:grid-cols-2 gap-4"><div className="bg-[var(--bg-input)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Pronóstico</p><p className="font-bold text-[var(--text-main)] mt-2">{pick.bet.market}</p><p className="text-[var(--accent)] font-bold mt-1">{pick.bet.selection} · @{Number(pick.bet.oddsAtPublication).toFixed(2)}</p></div><div className="bg-[var(--bg-input)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Horario</p><p className="font-bold text-[var(--text-main)] mt-2">Inicio: {formatOfficialDateTime(pick.event.kickoffAt)}</p><p className="text-[var(--accent)] text-sm mt-1">Publicado: {formatOfficialDateTime(pick.publishedAt)}</p></div></div><div className="border border-[var(--border)] rounded-2xl p-4"><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Evidencia</p><p className="text-sm text-[var(--text-main)] mt-2">Sistema: {pick.system.id} · versión {pick.system.version}</p><p className="text-xs font-mono break-all text-[var(--text-muted)] mt-2">SHA-256: {pick.source.evidenceHash}</p></div><OfficialPickTrustNote /><OfficialReviewStatus review={pick.review} /><div><p className="text-xs font-bold uppercase tracking-wider text-[var(--text-muted)] mb-3">Historial del registro</p><div className="space-y-2">{events.map((event) => <div key={event.id} className="flex items-center justify-between gap-3 text-sm bg-[var(--bg-input)] rounded-xl px-4 py-3"><div><p className="font-bold text-[var(--text-main)]">{eventLabel(event)}</p>{event.message && <p className="text-xs text-[var(--text-muted)] mt-1">{event.message}</p>}{event.permalink && <a href={event.permalink} target="_blank" rel="noreferrer" className="inline-block text-xs text-[var(--accent)] font-bold mt-1">Ver mensaje de Telegram</a>}</div><span className="text-[var(--text-muted)] text-xs shrink-0">{formatOfficialDateTime(event.createdAt)}</span></div>)}</div></div></div>}</div><footer className="p-6 border-t border-[var(--border)] space-y-3">{pick && <><a href={buildOfficialPickEntryLink(pick.id, { register: true })} className="block text-center bg-[var(--accent)] text-[var(--accent-fg)] rounded-xl py-3 font-bold">Crear cuenta y añadir a mi banca</a><a href={buildOfficialPickEntryLink(pick.id)} className="block text-center border border-[var(--border)] text-[var(--text-main)] rounded-xl py-3 font-bold">Ya tengo cuenta</a></>}<a href={window.location.pathname} className="block text-center text-sm text-[var(--text-muted)] font-bold">Ver MoneyTracKING</a></footer></section></main></>;
};

const PublicOfficialPicksPreview = () => {
    const [picks, setPicks] = useState([]);
    const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const response = await fetch('/api/official-picks?limit=3');
                const data = await response.json();
                if (response.ok && active) {
                    setPicks((Array.isArray(data.picks) ? data.picks : []).filter((pick) => !pick.isLocked));
                }
            } catch {
                // The login experience remains available even if the public feed is temporarily unavailable.
            } finally {
                if (active) setLoading(false);
            }
        };
    ...
    return <section className="bg-[var(--bg-base-95)] backdrop-blur-2xl p-6 md:p-8 rounded-[2rem] border border-[var(--border)] shadow-[var(--shadow-glow-lg)] w-full animate-in transition-colors"><div className="flex items-center gap-2 text-[var(--accent)] text-xs font-bold uppercase tracking-widest"><ShieldCheck size={16}/> Transparencia Money Tips</div><h2 className="text-2xl font-extrabold text-[var(--text-main)] mt-3">Últimos picks verificados</h2><p className="text-sm text-[var(--text-muted)] mt-2">Cada publicación conserva la hora y cuota registradas por el servidor. No garantiza resultados.</p>{loading ? <p className="py-8 text-sm text-[var(--text-muted)] text-center">Cargando comprobantes...</p> : picks.length === 0 ? <div className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--bg-input)] p-5 text-sm text-[var(--text-muted)]"><p className="font-bold text-[var(--text-main)]">Aún no hay picks oficiales publicados.</p><p className="mt-1">Cuando Money Tips publique uno, aparecerá aquí con su comprobante verificable.</p></div> : <div className="mt-6 space-y-3">{picks.map((pick) => <a key={pick.id} href={buildOfficialPickLink(pick.id)} className="block rounded-2xl border border-[var(--border)] bg-[var(--bg-input)] p-4 hover:border-[var(--accent-40)] transition-colors"><div className="flex items-center justify-between gap-3"><p className="font-bold text-[var(--text-main)] truncate">{pick.event.homeTeam} vs {pick.event.awayTeam}</p><span className="text-[var(--accent)] font-extrabold shrink-0">@{Number(pick.bet.oddsAtPublication).toFixed(2)}</span></div><p className="text-sm text-[var(--text-muted)] mt-1 truncate">Mercado y selección reservados para Premium</p><p className="text-xs text-[var(--text-muted)] mt-2">Publicado: {formatOfficialDateTime(pick.publishedAt)}</p></a>)}</div>}</section>;
};
...
const PremiumPayTestCheckoutPanel = ({ currentUser }) => {
    ...
    useEffect(() => {
        if (!isPremiumPayTestEnvironment || !currentUser) {
            setEntitlement(null);
            return undefined;
        }
    ...
    if (!isPremiumPayTestEnvironment || !currentUser) return null;
    ...
    return <section className="bg-[var(--accent-5)] border border-[var(--accent-30)] rounded-3xl p-5 md:p-7 space-y-4">
        <div><div className="flex items-center gap-2 text-[var(--accent)] text-xs font-bold uppercase tracking-widest"><ShieldCheck size={15}/> PremiumPay · entorno de prueba</div><h3 className="mt-2 text-xl font-extrabold text-[var(--text-main)]">Checkout técnico aislado</h3><p className="text-sm text-[var(--text-muted)] mt-1 max-w-2xl">Solo las cuentas autenticadas en este entorno aislado pueden abrir un checkout simulado. No crea cargos ni habilita acceso Premium hasta que PremiumPay comunique un pago de prueba completado.</p></div>
...
const OfficialPicksAdminPanel = ({ currentUser }) => {
    ...
    const removeKnownTestPick = async () => {
        const typedConfirmation = window.prompt('Esta acción elimina únicamente el registro de prueba Barcelona vs Madrid de la aplicación. Escribe ELIMINAR PRUEBA para continuar.');
        if (typedConfirmation !== 'ELIMINAR PRUEBA') return;
        if (!window.confirm('Última confirmación: se eliminará únicamente ese registro de prueba. No se podrá deshacer.')) return;

        setError('');
        setTestPickRemovalMessage('');
        setIsRemovingTestPick(true);
        try {
            const token = await currentUser.getIdToken();
            const response = await fetch('/api/admin/official-picks?mode=remove-known-test-pick', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                body: JSON.stringify({
                    pickId: 'op_a25e114b58ce96741d8f3f3edf8a8b757e3eaeaa',
                    confirmation: typedConfirmation
                })
            });
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'No se pudo retirar el registro de prueba.');
            setTestPickRemovalMessage('Registro de prueba retirado de MoneyTracKING. No había mensaje de Telegram asociado.');
        } catch (requestError) {
            setError(requestError.message || 'No se pudo retirar el registro de prueba.');
        } finally {
            setIsRemovingTestPick(false);
        }
    };
...
export default function App() {
    const [entryContext] = useState(() => {
        if (typeof window === 'undefined') return { pendingPickId: null, startsRegistering: false };
        const params = new URLSearchParams(window.location.search);
        const pendingPickId = params.get('addPick');
        return {
            pendingPickId: pendingPickId && /^[A-Za-z0-9_-]{16,100}$/.test(pendingPickId) ? pendingPickId : null,
            startsRegistering: params.get('register') === '1'
        };
    });
    ...
    const [isRegistering, setIsRegistering] = useState(entryContext.startsRegistering);
    ...
    const [customOptions, setCustomOptions] = useState({ sports: [], categories: [], tipsters: [] });
    ...
    const [pendingOfficialPickId, setPendingOfficialPickId] = useState(entryContext.pendingPickId);
    ...
    useEffect(() => {
        ...
        const subscriptionRef = doc(db, 'users', currentUser.uid, 'entitlements', 'subscription');
        return onSnapshot(subscriptionRef, (snapshot) => {
            const data = snapshot.exists() ? snapshot.data() : null;
            const expiresAt = data?.accessUntil?.toDate?.();
            const isActive = data?.status === 'active' || data?.status === 'trialing';
            setHasPremiumAccess(Boolean(isActive && expiresAt && expiresAt.getTime() > Date.now()));
        }, (error) => {
            console.error('Error cargando acceso Premium:', error);
            setHasPremiumAccess(false);
        });
    }, [currentUser, viewMode]);
    ...
    useEffect(() => {
        ...
        const unsubPrefs = onSnapshot(prefsRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                setCustomOptions({ sports: ['Fútbol', 'Baloncesto', 'Tenis', 'Esports'], categories: [], tipsters: [] });
            }
    ...
    useEffect(() => {
        if (currentUser && viewMode === 'personal' && pendingOfficialPickId && !loading) {
            setActiveTab('official-picks');
        }
    }, [currentUser, viewMode, pendingOfficialPickId, loading]);
    ...
    const clearPendingOfficialPick = () => {
        setPendingOfficialPickId(null);
    ...
    const handleCopyOfficialPick = async (pick) => {
        ...
        if (!canUsePremium) return showAlert('Necesitas Money Tips Premium para añadir picks activos a tu banca.');
        if (pick.isLocked || !pick.bet?.selection || !pick.bet?.oddsAtPublication) return showAlert('Este pick aún no está disponible para tu acceso.');
    ...
    const confirmAddBank = async (e) => {
        ...
        try {
            const bankRef = await addDoc(collection(db, 'users', currentUser.uid, 'banks'), newBank);
            setCurrentBankId(bankRef.id);
            localStorage.setItem(`moneytracking_last_bank_${currentUser.uid}`, bankRef.id);
            if (pendingOfficialPickId) setActiveTab('official-picks');
            setIsAddingBank(false);
        } catch(error) {
            console.error(error);
    ...
    if (isEmbed && activeBankData) {
        return (
            <div className="h-screen flex flex-col bg-[var(--bg-base)] text-[var(--text-main)] font-sans overflow-hidden w-full">
                ...
                <main className="flex-1 overflow-y-auto custom-scrollbar p-3 md:p-5 z-10 relative w-full">
                                ...
                                <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-4 h-64 flex flex-col relative shadow-sm transition-colors w-full">
                                    <div className="flex justify-between items-center mb-3">
                                        <div className="flex items-center gap-1.5 group relative cursor-help">
                                            ...
                                            <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-48 bg-[var(--bg-card)] border border-[var(--border-strong)] p-2 rounded-lg shadow-xl z-[60] text-[10px] text-[var(--text-main)] normal-case font-medium">
                                                Muestra el crecimiento de tu banca.<br/><span className="text-[var(--text-muted)]">Eje X: Secuencia de picks o Semanas.<br/>Eje Y: Beneficio Total acumulado.</span>
                                            </div>
                                        </div>
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        {chartViewMode === 'detailed' ? (
                                            <AreaChart data={stats.detailedChart}><defs><linearGradient id="colorProfit" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={theme === 'dark' ? '#5EE6B1' : '#2563EB'} stopOpacity={0.4}/><stop offset="95%" stopColor={theme === 'dark' ? '#5EE6B1' : '#2563EB'} stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'} vertical={false}/><XAxis dataKey="name" stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><YAxis stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><Tooltip contentStyle={{backgroundColor: theme === 'dark' ? 'rgba(17,22,33,0.95)' : 'rgba(255,255,255,0.95)', border:`1px solid ${theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, borderRadius:'8px', padding:'6px 10px', backdropFilter: 'blur(4px)'}} itemStyle={{color: theme === 'dark' ? '#5EE6B1' : '#2563EB', fontWeight:'bold', fontSize:'12px', padding:0}} labelStyle={{color: theme === 'dark' ? '#F1F5F9' : '#0F172A', fontSize:'10px', fontWeight:'bold', paddingBottom:'2px'}} formatter={(val)=>[formatCurrency(val,activeBankData?.currency), 'Beneficio']} /><Area type="monotone" dataKey="profit" stroke={theme === 'dark' ? '#5EE6B1' : '#2563EB'} strokeWidth={3} fill="url(#colorProfit)"/></AreaChart>
                                        ) : (
                                            <ReLineChart data={stats.weeklyChart}><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'} vertical={false}/><XAxis dataKey="name" stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><YAxis stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><Tooltip contentStyle={{backgroundColor: theme === 'dark' ? 'rgba(17,22,33,0.95)' : 'rgba(255,255,255,0.95)', border:`1px solid ${theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, borderRadius:'8px', padding:'6px 10px', backdropFilter: 'blur(4px)'}} itemStyle={{color: theme === 'dark' ? '#5EE6B1' : '#2563EB', fontWeight:'bold', fontSize:'12px', padding:0}} labelStyle={{color: theme === 'dark' ? '#F1F5F9' : '#0F172A', fontSize:'10px', fontWeight:'bold', paddingBottom:'2px'}} labelFormatter={(l, p) => p[0]?.payload?.fullLabel || l} formatter={(val)=>[formatCurrency(val,activeBankData?.currency), 'Beneficio']} /><Line type="linear" dataKey="profit" stroke={theme === 'dark' ? '#5EE6B1' : '#2563EB'} strokeWidth={3} dot={{r: 4, fill: theme === 'dark' ? '#5EE6B1' : '#2563EB', strokeWidth: 0}} activeDot={{r: 6, stroke: theme === 'dark' ? '#fff' : '#0F172A', strokeWidth: 2}}/></ReLineChart>
                                        )}
                                    </ResponsiveContainer>
                                </div>
                                {/* CHART 2 */}
                                <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-4 h-64 flex flex-col relative shadow-sm transition-colors w-full">
                                    <div className="flex justify-between items-center mb-3">
                                        <div className="flex items-center gap-1.5 group relative cursor-help">
                                            ...
                                            <HelpCircle size={12} className="text-[var(--text-muted)]" />
                                            <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-48 bg-[var(--bg-card)] border border-[var(--border-strong)] p-2 rounded-lg shadow-xl z-[60] text-[10px] text-[var(--text-main)] normal-case font-medium">
                                                Lo que ganas o pierdes en cada periodo.<br/><span className="text-[var(--text-muted)]">Eje X: Semana o Mes exacto.<br/>Eje Y: Beneficio de ese periodo.</span>
                                            </div>
                                        </div>
                                        <div className="flex bg-[var(--bg-input)] rounded-lg p-1 border border-[var(--border)]">
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={barChartViewMode === 'weekly' ? stats.weeklyBarChart : stats.monthlyBarChart}><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'} vertical={false}/><XAxis dataKey="name" stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><YAxis stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><Tooltip contentStyle={{backgroundColor: theme === 'dark' ? 'rgba(17,22,33,0.95)' : 'rgba(255,255,255,0.95)', border:`1px solid ${theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, borderRadius:'8px', padding:'6px 10px', backdropFilter: 'blur(4px)'}} itemStyle={{color: theme === 'dark' ? '#5EE6B1' : '#2563EB', fontWeight:'bold', fontSize:'12px', padding:0}} labelStyle={{color: theme === 'dark' ? '#F1F5F9' : '#0F172A', fontSize:'10px', fontWeight:'bold', paddingBottom:'2px'}} cursor={{fill: theme === 'dark' ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)'}} formatter={(val)=>[formatCurrency(val,activeBankData?.currency), 'Beneficio']}/><Bar dataKey="profit" radius={[4,4,0,0]}>{(barChartViewMode === 'weekly' ? stats.weeklyBarChart : stats.monthlyBarChart).map((e,i)=><Cell key={`c-${i}`} fill={e.profit>=0?(theme === 'dark' ? '#5EE6B1' : '#2563EB'):(theme === 'dark' ? '#FF5A5F' : '#EF4444')}/>)}</Bar></BarChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
                        </div>
                    ...
                    {activeTab === 'bets' && (
                        <div className="space-y-3 w-full">
                            {betsByMonth.map(g => (
                                <div key={g.id} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm w-full">
                                    ...
                                    {expandedMonths[g.id] && (
                                        <div className="divide-y divide-[var(--border)] w-full">
                                            {g.bets.map(b => {
    ...
    if (!currentUser && viewMode === 'personal') {
        return (
            ...
            <div className="min-h-screen relative flex items-center justify-center p-4 md:p-8">
                <div className="w-full max-w-5xl grid gap-5 lg:grid-cols-[400px,minmax(0,1fr)] items-start">
    ...
    if (banks.length === 0 && viewMode === 'personal') {
        return (
            <><style>{getGlobalStyles(theme)}</style>
            <LiquidBackground theme={theme} />
            <div className="flex flex-col md:flex-row h-screen text-[var(--text-main)] font-sans overflow-hidden w-full">
                ...
                <main className="flex-1 flex flex-col items-center justify-center p-6 text-center w-full">
                    <div className="bg-[var(--bg-card)] backdrop-blur-2xl p-8 rounded-3xl border border-[var(--border)] shadow-[var(--shadow-glow-md)] max-w-lg w-full transition-colors">
                        ...
                        <p className="text-[var(--text-muted)] mb-6 leading-relaxed">{pendingOfficialPickId ? 'Tu pick oficial sigue reservado. Crea tu primera banca y volverás a él para revisarlo y añadirlo.' : 'Tus datos ahora están seguros en la nube. Configura tu primera banca para empezar a trackear como un profesional.'}</p>
                        {pendingOfficialPickId && <div className="mb-6 text-left bg-[var(--accent-10)] border border-[var(--accent-30)] rounded-2xl p-4 text-sm"><p className="font-bold text-[var(--accent)]">Siguiente paso: configura tu capital.</p><p className="text-[var(--text-muted)] mt-1">La aplicación no añadirá ninguna apuesta automáticamente.</p></div>}
    ...
    return (
        ...
        <div className="flex flex-col md:flex-row h-screen text-[var(--text-main)] font-sans overflow-hidden w-full">
            {isMobileMenuOpen && (
                <div className="fixed inset-0 z-50 bg-[var(--bg-modal)] backdrop-blur-md md:hidden animate-in fade-in w-full" onClick={() => setIsMobileMenuOpen(false)}>
            ...
            <aside className={`hidden md:flex flex-col w-64 bg-[var(--bg-card)]/50 backdrop-blur-2xl border-r border-[var(--border)] transition-colors shrink-0`}>
                ...
                {viewMode === 'visiting' ? (
                    <div className="flex-1 flex flex-col p-4 bg-indigo-500/5 w-full">
                        <div className="mb-4 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl backdrop-blur-md w-full"><p className="text-[10px] text-indigo-500 dark:text-indigo-300 uppercase font-bold mb-1 flex items-center gap-2"><Eye size={12}/> Modo Visitante</p><p className="text-[var(--text-main)] text-sm font-bold truncate drop-shadow-sm">{activeBankData?.name}</p></div>
                        <nav className="space-y-2 w-full"><button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeTab === 'dashboard' ? 'bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border border-indigo-500/30 shadow-lg' : 'text-[var(--text-muted)] hover:bg-[var(--bg-overlay)] hover:text-[var(--text-main)]'}`}><LayoutDashboard size={18}/> Dashboard</button><button onClick={() => setActiveTab('bets')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeTab === 'bets' ? 'bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border border-indigo-500/30 shadow-lg' : 'text-[var(--text-muted)] hover:bg-[var(--bg-overlay)] hover:text-[var(--text-main)]'}`}><List size={18}/> Historial</button></nav>
                        <div className="mt-auto w-full"><button onClick={handleExitVisiting} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold text-[var(--text-main)] bg-[var(--bg-overlay)] border border-[var(--border)] hover:bg-[var(--bg-overlay-hover)] transition-all backdrop-blur-md"><LogOut size={16}/> {currentUser ? 'Volver a mi Banca' : 'Ir al Inicio de Sesión'}</button></div>
                    </div>
                ) : (
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative mb-16 md:mb-0 w-full">
                <header className="h-16 bg-transparent backdrop-blur-xl border-b border-[var(--border)] flex items-center justify-between px-6 sticky top-0 z-30 transition-colors w-full">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {viewMode === 'personal' && (banks.length > 0 || balances.length > 0) && (
                            <div className="relative hidden sm:block w-[200px] shrink-0">
                        ...
                        {viewMode === 'visiting' && (
                            <div className="bg-indigo-500/20 border border-indigo-500/30 px-3 py-1.5 rounded-lg text-xs text-indigo-600 dark:text-indigo-300 font-bold backdrop-blur-sm shrink-0" title="Por privacidad, las apuestas pendientes están ocultas.">Modo Lectura</div>
                        )}
                    </div>
                </header>
                ...
                {activeTab === 'dashboard' && activeBankData && (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full"><div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-5 h-60 md:h-[260px] flex flex-col relative shadow-sm transition-colors w-full"><div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-1.5 group relative cursor-help">
                            <h4 className="text-xs text-[var(--text-muted)] uppercase font-bold tracking-wider">Evolución del Beneficio</h4>
                            <HelpCircle size={14} className="text-[var(--text-muted)]" />
                            <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-56 bg-[var(--bg-card)] border border-[var(--border-strong)] p-3 rounded-xl shadow-xl z-[60] text-[11px] text-[var(--text-main)] normal-case font-medium leading-relaxed">
                                Muestra el crecimiento de tu banca.<br/><span className="text-[var(--text-muted)]">Eje X: Secuencia de picks o Semanas.<br/>Eje Y: Beneficio Total acumulado.</span>
                            </div>
                        </div>
                        <div className="flex bg-[var(--bg-input)] rounded-lg p-1 border border-[var(--border)] backdrop-blur-sm"><button onClick={() => setChartViewMode('detailed')} className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${chartViewMode==='detailed'?'bg-[var(--accent)] text-[var(--accent-fg)] shadow-[var(--shadow-glow-sm)]':'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}><TrendingUp size={14}/></button><button onClick={() => setChartViewMode('weekly')} className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${chartViewMode==='weekly'?'bg-[var(--accent)] text-[var(--accent-fg)] shadow-[var(--shadow-glow-sm)]':'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}><LineChart size={14}/></button></div></div><ResponsiveContainer width="100%" height="100%">{chartViewMode === 'detailed' ? (<AreaChart data={stats.detailedChart}><defs><linearGradient id="colorProfit" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={theme === 'dark' ? '#5EE6B1' : '#2563EB'} stopOpacity={0.4}/><stop offset="95%" stopColor={theme === 'dark' ? '#5EE6B1' : '#2563EB'} stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'} vertical={false}/><XAxis dataKey="name" stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><YAxis stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><Tooltip contentStyle={{backgroundColor: theme === 'dark' ? 'rgba(17,22,33,0.95)' : 'rgba(255,255,255,0.95)', border:`1px solid ${theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, borderRadius:'8px', padding:'6px 10px', backdropFilter: 'blur(4px)'}} itemStyle={{color: theme === 'dark' ? '#5EE6B1' : '#2563EB', fontWeight:'bold', fontSize:'12px', padding:0}} labelStyle={{color: theme === 'dark' ? '#F1F5F9' : '#0F172A', fontSize:'10px', fontWeight:'bold', paddingBottom:'2px'}} formatter={(val)=>[formatCurrency(val,activeBankData?.currency), 'Beneficio']} /><Area type="monotone" dataKey="profit" stroke={theme === 'dark' ? '#5EE6B1' : '#2563EB'} strokeWidth={3} fill="url(#colorProfit)"/></AreaChart>) : (<ReLineChart data={stats.weeklyChart}><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'} vertical={false}/><XAxis dataKey="name" stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><YAxis stroke={theme === 'dark' ? '#64748b' : '#94a3b8'} tick={{fontSize:10, fill: theme === 'dark' ? '#64748b' : '#94a3b8'}} tickLine={false} axisLine={false}/><Tooltip contentStyle={{backgroundColor: theme === 'dark' ? 'rgba(17,22,33,0.95)' : 'rgba(255,255,255,0.95)', border:`1px solid ${theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, borderRadius:'8px', padding:'6px 10px', backdropFilter: 'blur(4px)'}} itemStyle={{color: theme === 'dark' ? '#5EE6B1' : '#2563EB', fontWeight:'bold', fontSize:'12px', padding:0}} labelStyle={{color: theme === 'dark' ? '#F1F5F9' : '#0F172A', fontSize:'10px', fontWeight:'bold', paddingBottom:'2px'}} labelFormatter={(l, p) => p[0]?.payload?.fullLabel || l} formatter={(val)=>[formatCurrency(val,activeBankData?.currency), 'Beneficio']} /><Line type="linear" dataKey="profit" stroke={theme === 'dark' ? '#5EE6B1' : '#2563EB'} strokeWidth={3} dot={{r: 4, fill: theme === 'dark' ? '#5EE6B1' : '#2563EB', strokeWidth: 0}} activeDot={{r: 6, stroke: theme === 'dark' ? '#fff' : '#0F172A', strokeWidth: 2}}/></ReLineChart>)}</ResponsiveContainer></div><div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-5 h-60 md:h-[260px] flex flex-col relative shadow-sm transition-colors w-full"><div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-1.5 group relative cursor-help">
                            <h4 className="text-xs text-[var(--text-muted)] uppercase font-bold tracking-wider">Rendimiento Temporal</h4>
                            <HelpCircle size={14} className="text-[var(--text-muted)]" />
                            <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-56 bg-[var(--bg-card)] border border-[var(--border-strong)] p-3 rounded-xl shadow-xl z-[60] text-[11px] text-[var(--text-main)] normal-case font-medium leading-relaxed">
                                Lo que ganas o pierdes en cada periodo.<br/><span className="text-[var(--text-muted)]">Eje X: Semana o Mes exacto.<br/>Eje Y: Beneficio neto de ese periodo.</span>
                            </div>
                        </div>
                ...
                {activeTab === 'bets' && (
                            ...
                            {activeBankData?.premiumPassword ? (
                                <>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-3 max-w-sm mx-auto w-full">
                                        <input type="password" value={visitorPasswordInput} onChange={e=>setVisitorPasswordInput(e.target.value)} className="flex-1 bg-[var(--bg-input)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text-main)] outline-none focus:border-[var(--accent)] shadow-inner transition-colors w-full" placeholder="Contraseña..." />
                                        <button onClick={() => { if(visitorPasswordInput === activeBankData.premiumPassword) { setUnlockedBank(true); showAlert('✅ Acceso Concedido'); } else { showAlert('Contraseña incorrecta'); } }} className="bg-[var(--accent)] text-[var(--accent-fg)] px-6 py-3 rounded-xl font-bold hover:bg-[var(--accent-hover)] transition-all shadow-md w-full sm:w-auto">Desbloquear</button>
                                    </div>
                                </>
                            ) : (
                    ...
                    {activeBetsData.length===0?<div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-12 text-center text-[var(--text-muted)] text-sm shadow-sm transition-colors w-full">El historial está vacío. {viewMode === 'personal' && !activeBankData?.isBalance ? 'Escanea un boleto o añade una apuesta para empezar a ver datos.' : ''}</div>:
                ...
                {activeTab === 'balances' && viewMode === 'personal' && (
                    <div className="w-full space-y-6 pb-20 md:pb-0">
                        ...
                        {balances.length === 0 ? (
                            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-12 text-center text-[var(--text-muted)] text-sm shadow-sm transition-colors w-full">
                            ...
                            <div className="grid grid-cols-1 gap-6 w-full">
                                {balances.map(balance => {
                                    ...
                                    return (
                                        <div key={balance.id} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6 shadow-sm transition-all hover:border-[var(--accent-50)] group w-full">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
                                                <div className="bg-[var(--bg-input)] rounded-2xl p-4 text-center border border-[var(--border)] w-full">
                    ...
                    <div className="w-full space-y-6 pb-20 md:pb-0">
                                    ...
                                    <div key={bank.id} className={`relative p-6 rounded-2xl border transition-all duration-300 ${bank.id === currentBankId ? 'bg-[var(--bg-hover)] border-[var(--accent-40)] shadow-[var(--shadow-glow-sm)]' : 'bg-[var(--bg-overlay)] border-[var(--border)] hover:border-[var(--border-strong)]'}`}>
                                        ...
                                        <div className="flex flex-col sm:flex-row justify-between gap-6">
                                            <div className="flex-1 space-y-5">
                                                <div className="space-y-1"><label className="text-xs text-[var(--text-muted)] uppercase font-bold tracking-wider ml-1">Nombre</label><input type="text" defaultValue={bank.name} onBlur={(e) => { if (e.target.value !== bank.name) handleUpdateBank(bank.id, 'name', e.target.value); }} className="w-full bg-transparent border-b border-[var(--border-strong)] focus:border-[var(--accent)] text-xl font-bold text-[var(--text-main)] outline-none py-1.5 transition-colors placeholder-[var(--text-muted)]" /></div>
                                                <div className="flex flex-wrap gap-4">
                                                    <div className="flex-1 min-w-[100px] space-y-1"><label className="text-xs text-[var(--text-muted)] uppercase font-bold tracking-wider ml-1 flex items-center gap-1">Capital <Lock size={10}/></label><input type="number" value={bank.initialCapital} disabled className="w-full bg-[var(--bg-base)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-[var(--text-muted)] font-bold outline-none transition-colors text-sm cursor-not-allowed" /></div>
                                                    <div className="w-24 space-y-1"><label className="text-xs text-[var(--text-muted)] uppercase font-bold tracking-wider ml-1">Divisa</label><select value={bank.currency || 'EUR'} onChange={(e) => { handleUpdateBank(bank.id, 'currency', e.target.value); }} className="w-full bg-[var(--bg-base)] border border-[var(--border)] rounded-xl px-2 py-2.5 text-[var(--text-main)] text-sm outline-none appearance-none"><option value="EUR">EUR (€)</option><option value="USD">USD ($)</option><option value="GBP">GBP (£)</option><option value="MXN">MXN ($)</option></select></div>
                                                    <div className="flex-1 min-w-[130px] space-y-1"><label className="text-xs text-[var(--accent)] uppercase font-bold tracking-wider ml-1 flex items-center gap-1 drop-shadow-sm"><Lock size={12}/> Privacidad</label><input type="text" defaultValue={bank.premiumPassword || ''} onBlur={(e) => { handleUpdateBank(bank.id, 'premiumPassword', e.target.value); }} placeholder="Clave para ver pendientes..." className="w-full bg-[var(--bg-base)] border border-[var(--accent-30)] focus:border-[var(--accent)] rounded-xl px-3 py-2.5 text-[var(--text-main)] text-sm outline-none transition-colors shadow-inner" /></div>
                                                </div>
                                            </div>
                                            <div className="flex flex-row sm:flex-col justify-between items-end gap-3 border-t sm:border-t-0 sm:border-l border-[var(--border)] pt-5 sm:pt-0 sm:pl-6 min-w-[140px]">
                                                <button onClick={() => handleBankChange({target: {value: bank.id}})} disabled={bank.id === currentBankId} className={`w-full px-4 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${bank.id === currentBankId ? 'bg-[var(--accent-10)] text-[var(--accent)] cursor-default border border-[var(--accent-30)]' : 'bg-[var(--bg-overlay)] border border-[var(--border)] text-[var(--text-main)] hover:bg-[var(--accent)] hover:text-[var(--accent-fg)] hover:border-transparent'}`}>{bank.id === currentBankId ? 'Operando' : 'Seleccionar'}</button>
                                                <button onClick={() => handleDeleteBank(bank.id)} className="w-full px-4 py-3 text-xs font-bold text-[var(--text-muted)] hover:text-[var(--red)] hover:bg-[var(--red-10)] rounded-xl transition-all flex items-center justify-center gap-2 border border-transparent"><Trash2 size={16}/> Eliminar</button>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6 lg:p-8 shadow-sm transition-colors">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                <button onClick={handleExportCSV} className="flex-1 bg-[var(--bg-base)] hover:bg-[var(--bg-hover)] border border-[var(--border)] hover:border-[var(--accent-30)] text-[var(--text-main)] p-6 rounded-2xl transition-all group flex flex-col items-center gap-3 shadow-inner w-full"><div className="p-4 bg-[var(--bg-overlay)] rounded-2xl group-hover:bg-[var(--accent-10)] transition-colors"><Download size={28} className="text-[var(--text-muted)] group-hover:text-[var(--accent)]" /></div><span className="text-sm font-bold tracking-wide">Descargar CSV</span></button>
                                <div className="flex-1 relative w-full"><input type="file" accept=".csv" ref={fileInputRef} onChange={handleImportCSV} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" /><div className="bg-[var(--bg-base)] hover:bg-[var(--bg-hover)] border border-[var(--border)] hover:border-[var(--accent-30)] text-[var(--text-main)] p-6 rounded-2xl transition-all group flex flex-col items-center gap-3 h-full shadow-inner w-full"><div className="p-4 bg-[var(--bg-overlay)] rounded-2xl group-hover:bg-[var(--accent-10)] transition-colors"><Upload size={28} className="text-[var(--text-muted)] group-hover:text-[var(--accent)]" /></div><span className="text-sm font-bold tracking-wide">Importar Datos</span></div></div>
                            </div>
                        </div>
                    </div>
                )}
                        ...
                        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6 lg:p-8 shadow-sm transition-colors">
                            ...
                            <div className="space-y-8 lg:grid lg:grid-cols-2 lg:gap-8 lg:space-y-0">
                                <div className="bg-[var(--bg-base)] p-6 rounded-2xl border border-[var(--border)] shadow-inner transition-colors">
                                    ...
                                    <div className="flex gap-3 mb-5"><input type="text" value={newCustomSport} onChange={e => setNewCustomSport(e.target.value)} placeholder="Ej: Ping Pong, Dardos..." className="flex-1 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text-main)] text-sm outline-none focus:border-[var(--accent-50)] w-full"/><button onClick={() => handleAddOption('sports', newCustomSport)} className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--accent-fg)] p-3 rounded-xl font-bold shadow-[var(--shadow-glow-sm)] transition-all shrink-0"><Plus size={20}/></button></div>
                                ...
                                <div className="bg-[var(--bg-base)] p-6 rounded-2xl border border-[var(--border)] shadow-inner transition-colors">
                                    ...
                                    <div className="flex gap-3 mb-5"><input type="text" value={newCustomCategory} onChange={e => setNewCustomCategory(e.target.value)} placeholder="Ej: Reto Escalera, Stake 10..." className="flex-1 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text-main)] text-sm outline-none focus:border-[var(--accent-50)] w-full"/><button onClick={() => handleAddOption('categories', newCustomCategory)} className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--accent-fg)] p-3 rounded-xl font-bold shadow-[var(--shadow-glow-sm)] transition-all shrink-0"><Plus size={20}/></button></div>
                ...
                <div className="bg-[var(--bg-base-95)] backdrop-blur-3xl w-full max-w-lg rounded-3xl shadow-[var(--shadow-glow-lg)] border border-[var(--accent-30)] overflow-hidden flex flex-col max-h-[95vh] transition-colors">
                    ...
                    <div className="p-6 overflow-y-auto space-y-5 custom-scrollbar bg-transparent w-full">
                        ...
                        {newBet.selections.map((sel, index) => (
                            <div key={sel.id} className="border border-[var(--border)] rounded-2xl bg-[var(--bg-card)] overflow-hidden shadow-sm transition-colors w-full">
                                <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-[var(--bg-hover)] transition-colors border-b border-[var(--border)] w-full" onClick={() => toggleSelection(sel.id)}><div className="flex items-center gap-2 text-sm text-[var(--accent)] font-bold tracking-wide"><ChevronDown size={16} className={`transform transition-transform ${sel.isOpen ? 'rotate-180' : ''}`} /> Selección {index + 1}</div>{newBet.selections.length > 1 && (<button onClick={(e) => { e.stopPropagation(); handleRemoveSelection(sel.id); }} className="text-[var(--text-muted)] hover:text-[var(--red)] bg-[var(--bg-overlay)] p-1.5 rounded-lg"><X size={14}/></button>)}</div>
                                {sel.isOpen && (
                                    <div className="p-5 space-y-4 animate-in slide-in-from-top-2 bg-[var(--bg-base)] w-full">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full"><div className="md:col-span-2 space-y-1.5"><label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Evento</label><input type="text" placeholder="Ej: RM - FCB" className="w-full bg-[var(--bg-card)] border border-transparent rounded-xl px-3 py-2.5 text-[var(--text-main)] text-sm outline-none focus:border-[var(--accent-50)] shadow-inner" value={sel.title} onChange={e => handleUpdateSelection(sel.id, 'title', e.target.value)} /></div><div className="space-y-1.5"><label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Cuota</label><input type="number" step="0.01" className="w-full bg-[var(--bg-card)] border border-transparent rounded-xl px-3 py-2.5 text-[var(--accent)] font-extrabold text-sm outline-none focus:border-[var(--accent-50)] shadow-inner" value={sel.odds} onChange={e => handleUpdateSelection(sel.id, 'odds', e.target.value)} /></div></div>
                                        <div className="space-y-1.5 w-full"><label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Pronóstico</label><input type="text" placeholder="Ej: Más de 2.5 Goles..." className="w-full bg-[var(--bg-card)] border border-transparent rounded-xl px-3 py-2.5 text-[var(--text-main)] text-sm outline-none focus:border-[var(--accent-50)] shadow-inner" value={sel.selection} onChange={e => handleUpdateSelection(sel.id, 'selection', e.target.value)} /></div>
                                        <div className="grid grid-cols-2 gap-4 w-full">
                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Deporte</label>
                                                <select className="w-full bg-[var(--bg-card)] border border-transparent rounded-xl px-3 py-2.5 text-[var(--text-main)] text-sm outline-none shadow-inner appearance-none" value={sel.sport} onChange={e => handleUpdateSelection(sel.id, 'sport', e.target.value)}>{(customOptions.sports || []).map(s => <option key={s} value={s}>{s}</option>)}{(!customOptions.sports || customOptions.sports.length === 0) && <option value="">Sin deportes definidos</option>}</select>
                                            </div>
                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Estado</label>
                                                <select className="w-full bg-[var(--bg-card)] border border-transparent rounded-xl px-3 py-2.5 text-[var(--text-main)] text-sm outline-none shadow-inner appearance-none font-bold" value={sel.status} onChange={e => handleUpdateSelection(sel.id, 'status', e.target.value)}>
                                                    <option value="pending" className="text-[var(--yellow)]">Pendiente</option>
                                                    <option value="won" className="text-[var(--accent)]">Ganada</option>
                                                    <option value="lost" className="text-[var(--red)]">Perdida</option>
                                                    <option value="void">Reembolsada</option>
                                                    <option value="half-won" className="text-[var(--accent)]">Mitad ganada</option>
                                                    <option value="half-lost" className="text-[var(--red)]">Mitad perdida</option>
                                                    <option value="cancelled" className="text-[var(--text-muted)]">Cancelada</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div className="flex justify-center pt-3 border-t border-[var(--border)] w-full"><button className="text-xs font-bold text-[var(--text-muted)] flex items-center gap-1 hover:text-[var(--text-main)] transition-colors" onClick={(e) => { e.stopPropagation(); toggleSelection(sel.id); }}><ChevronUp size={14}/> Ocultar detalles</button></div>
                                    </div>
                                )}
                            </div>
                        ))}
                        ...
                        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-5 space-y-5 shadow-inner transition-colors w-full">
                            ...
                            <div className="space-y-3 w-full">
                                <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1 flex justify-between">Inversión {formErrors.amount && <span className="text-[var(--red)] text-[10px] flex items-center gap-1"><AlertTriangle size={10}/> {formErrors.amount}</span>}</label>
                                <div className="flex gap-4 w-full">
                                    <div className={`flex-1 bg-[var(--bg-base)] border ${formErrors.amount ? 'border-[var(--red-50)]' : 'border-transparent'} rounded-xl p-3 relative group focus-within:border-[var(--accent-50)] transition-colors shadow-inner`}>
                                        <span className="text-[10px] text-[var(--text-muted)] font-bold block uppercase tracking-widest mb-1">Importe ({currencySymbol})</span>
                                        <input type="number" className="bg-transparent text-[var(--text-main)] w-full outline-none font-extrabold text-2xl" value={newBet.amount || ''} onChange={e => handleAmountChange(e.target.value, 'amount')} placeholder="0" />
                                    </div>
                                    <div className="flex-1 bg-[var(--bg-base)] border border-transparent rounded-xl p-3 relative group focus-within:border-[var(--accent-50)] transition-colors shadow-inner">
                                        <span className="text-[10px] text-[var(--text-muted)] font-bold block uppercase tracking-widest mb-1">% Capital</span>
                                        <input type="number" className="bg-transparent text-[var(--text-main)] w-full outline-none font-extrabold text-2xl" value={newBet.stake || ''} onChange={e => handleAmountChange(e.target.value, 'stake')} placeholder="0" />
                                    </div>
                                </div>
                                <div className="grid grid-cols-4 gap-2 mt-3 w-full">
                                    {quickStakes.map(perc => {
                                        const val = (bankCapForButtons * (perc/100)).toFixed(0);
                                        return <button key={perc} onClick={() => handleAmountChange(val, 'amount')} className="bg-[var(--bg-overlay)] hover:bg-[var(--bg-overlay-hover)] border border-transparent rounded-lg py-2 text-xs font-bold text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors shadow-sm">{val}{currencySymbol}</button>
                                    })}
                                </div>
                            </div>
                            
                            <button onClick={() => setShowMoreOptions(!showMoreOptions)} className="w-full text-center text-xs font-bold text-[var(--text-muted)] hover:text-[var(--text-main)] flex items-center justify-center gap-2 mt-2 transition-colors border-t border-[var(--border)] pt-4">{showMoreOptions ? <ChevronUp size={14}/> : <ChevronDown size={14}/>} {showMoreOptions ? 'Cerrar Avanzado' : 'Más opciones'}</button>
                            
                            {showMoreOptions && (
                                <div className="space-y-4 pt-2 animate-in fade-in slide-in-from-top-2 w-full">
                                    <div className="grid grid-cols-2 gap-4 w-full">
                                        <div className="space-y-1.5"><label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1 flex items-center gap-1"><User size={10}/> Tipster</label><input type="text" className="w-full bg-[var(--bg-base)] border border-transparent rounded-xl px-3 py-2 text-[var(--text-main)] text-sm outline-none shadow-inner" value={newBet.tipster || ''} onChange={e => setNewBet({...newBet, tipster: e.target.value})} placeholder="Nombre..."/></div>
                                        <div className="space-y-1.5"><label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Estrategia</label><select className="w-full bg-[var(--bg-base)] border border-transparent rounded-xl px-3 py-2 text-[var(--text-main)] text-sm outline-none shadow-inner appearance-none" value={newBet.selections[0]?.category || ''} onChange={e => handleUpdateSelection(newBet.selections[0]?.id, 'category', e.target.value)}><option value="">Sin categoría</option>{(customOptions.categories || []).map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4 w-full">
                                        ...
                                        <div className="space-y-1.5"><label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-1">Cashout (€)</label><input type="number" className="w-full bg-[var(--bg-base)] border border-transparent rounded-xl px-3 py-2 text-[var(--text-main)] text-sm outline-none shadow-inner" value={newBet.cashout || ''} onChange={e => setNewBet({...newBet, cashout: e.target.value})} placeholder="0.00"/></div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3 w-full">
                                        <div className="flex items-center gap-3 bg-[var(--bg-base)] px-3 py-2.5 rounded-xl shadow-inner w-full"><input type="checkbox" checked={newBet.isLive} onChange={e => setNewBet({...newBet, isLive: e.target.checked})} className="accent-[var(--accent)] w-4 h-4 shrink-0"/><label className="text-xs font-bold text-[var(--text-main)] truncate">En Vivo</label></div>
                                        <div className="flex items-center gap-3 bg-[var(--bg-base)] px-3 py-2.5 rounded-xl shadow-inner w-full"><input type="checkbox" checked={newBet.isFreebet} onChange={e => setNewBet({...newBet, isFreebet: e.target.checked})} className="accent-pink-500 w-4 h-4 shrink-0"/><label className="text-xs font-bold text-[var(--text-main)] truncate">Freebet</label></div>
                                        <div className="flex items-center gap-3 bg-[var(--bg-base)] px-3 py-2.5 rounded-xl shadow-inner w-full"><input type="checkbox" checked={newBet.isEachWay || false} onChange={e => setNewBet({...newBet, isEachWay: e.target.checked})} className="accent-[var(--accent)] w-4 h-4 shrink-0"/><label className="text-xs font-bold text-[var(--text-main)] truncate">Each-Way</label></div>
                                        <div className="flex items-center gap-3 bg-[var(--bg-base)] px-3 py-2.5 rounded-xl shadow-inner w-full"><input type="checkbox" checked={newBet.bonus ? true : false} onChange={e => setNewBet({...newBet, bonus: e.target.checked ? '1' : ''})} className="accent-yellow-500 w-4 h-4 shrink-0"/><label className="text-xs font-bold text-[var(--text-main)] truncate">Bono</label></div>
                                    </div>
                                    
                                    <div className="flex items-start gap-3 bg-[var(--bg-base)] px-3 py-3 rounded-xl shadow-inner border border-transparent hover:border-[var(--accent-20)] transition-colors w-full">
                                        <input type="checkbox" checked={newBet.isHidden || false} onChange={e => setNewBet({...newBet, isHidden: e.target.checked})} className="accent-[var(--accent)] w-4 h-4 mt-0.5 shrink-0"/>
                                        <div>
                                            <label className="text-xs font-bold text-[var(--text-main)] block">Ocultar temporalmente</label>
                                            <p className="text-[10px] text-[var(--text-muted)] mt-1 leading-snug">Se oculta en el enlace público durante 48h si está "Pendiente".</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[var(--bg-modal)] backdrop-blur-md animate-in fade-in w-full">
                    <div className="bg-[var(--bg-card)] backdrop-blur-2xl rounded-3xl shadow-2xl border border-[var(--accent-30)] p-6 md:p-8 w-full max-w-lg transition-colors overflow-hidden relative">
                        ...
                        <div className="flex items-center gap-3 mb-6 relative z-10 w-full">
                            <div className="w-12 h-12 bg-[var(--accent-10)] rounded-xl flex items-center justify-center text-[var(--accent)] shadow-inner border border-[var(--accent-20)] shrink-0"><Code size={24}/></div>
                            <div className="flex-1 min-w-0">
                                <h3 className="text-[var(--text-main)] font-extrabold text-xl tracking-tight truncate">Insertar o Compartir</h3>
                                <p className="text-[var(--text-muted)] text-sm truncate">Elige cómo quieres distribuir tu banca.</p>
                            </div>
                        </div>
                                ...
                                <div className="max-h-40 overflow-y-auto space-y-2 custom-scrollbar pr-2 w-full">
                                    {banks.map(b => (
                                        <label key={b.id} className="flex items-center gap-3 p-3 bg-[var(--bg-card)] rounded-xl border border-[var(--border)] cursor-pointer hover:bg-[var(--bg-hover)] transition-colors w-full">
                                            ...
                                            />
                                            <span className="text-sm font-bold text-[var(--text-main)] truncate">{b.name}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
            ...
            {statusModalData && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-[var(--bg-modal)] backdrop-blur-md animate-in fade-in w-full">
                    <div className="bg-[var(--bg-base-95)] backdrop-blur-2xl rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.2)] border border-[var(--accent-20)] p-6 md:p-8 w-full max-w-sm transition-colors">
                        <h3 className="text-[var(--text-main)] font-extrabold text-xl mb-6 text-center tracking-tight drop-shadow-sm">Estado de la Operación</h3>
                        <div className="grid grid-cols-2 gap-3 md:gap-4 mb-4 w-full">
                            <button onClick={() => handleQuickStatusChange('won')} className="p-3 md:p-4 bg-[var(--accent-10)] hover:bg-[var(--accent-20)] text-[var(--accent)] rounded-2xl font-bold border border-[var(--accent-30)] transition-all flex flex-col items-center gap-2 shadow-inner w-full"><CheckCircle2 size={24}/> Ganada</button>
                            <button onClick={() => handleQuickStatusChange('lost')} className="p-3 md:p-4 bg-[var(--red-10)] hover:bg-[var(--red-20)] text-[var(--red)] rounded-2xl font-bold border border-[var(--red-30)] transition-all flex flex-col items-center gap-2 shadow-inner w-full"><XCircle size={24}/> Perdida</button>
                        </div>
                        <div className="grid grid-cols-2 gap-2 mb-4 w-full">
                            <button onClick={() => handleQuickStatusChange('half-won')} className="py-2 text-[10px] bg-[var(--accent-10)] hover:bg-[var(--accent-20)] text-[var(--accent)] rounded-lg font-bold border border-[var(--accent-30)] transition-all w-full">Mitad Ganada</button>
                            <button onClick={() => handleQuickStatusChange('half-lost')} className="py-2 text-[10px] bg-[var(--red-10)] hover:bg-[var(--red-20)] text-[var(--red)] rounded-lg font-bold border border-[var(--red-30)] transition-all w-full">Mitad Perdida</button>
                        </div>
